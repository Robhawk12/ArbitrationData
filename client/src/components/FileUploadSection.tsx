...
export default function FileUploadSection({ 
  ...
  const uploadFile = async (file: File) => {
    ...
    // Validate file size (5KB - 50MB)
    if (file.size < 5 * 1024) {
      onFileError('File is too small. Min size is 5KB');
      return;
    }
    ...
    console.log('Attempting to upload file:', file.name, 'Size:', (file.size / (1024 * 1024)).toFixed(2), 'MB');
    ...
    try {
      ...
      xhr.onerror = () => {
        ...
        setIsUploading(false);
        setProcessingFile(null);
        onFileError('Network error occurred during file upload. Please try again or use a smaller file.');
      };
      
      xhr.ontimeout = () => {
        console.error("XHR timeout during file upload");
        setIsUploading(false);
        setProcessingFile(null);
        onFileError('Upload timed out. Please try again with a smaller file or check your network connection.');
      };
      ...
      <div className="bg-white rounded-md shadow-sm p-4">
        ...
        >
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-[8pt] text-neutral-300">Supported formats: .xlsx, .xls (up to 50MB)</p>
